...
*/
var isFree = courses.every(function (currentValue, index) {
    // Chỉ cần phần tử nào trả về false thì sẽ lập tức dừng lại
    console.log(index);
    return currentValue.coin === 0;
});
console.log(isFree);
console.log('________________________');
...
*/
var isCost = courses.some(function (currentValue, index) {
    // Chỉ cần phần tử nào trả về true thì sẽ lập tức dừng lại
    return currentValue.coin > 0;
});
console.log(isCost);
console.log('________________________');
...
*/
var isFind = courses.find(function (currentValue, index) {
        return currentValue.name === 'JS';
    }
);
console.log(isFind);
console.log('________________________');
...
*/
var isFilter = courses.filter(function (currentValue, index) {
        return currentValue.name === 'JS';
    }
);
...
var newCourses = courses.map(function (course) {
    return {
        ...
        coin: course.coin == 0 ? "Free" : course.coin
    };
});
...
/*
    Biến mảng lồng mảng thành 1 mảng. Vì thế initial value sẽ là 1 mảng trống []
*/
var depthArr = [1, 2, [3, 4], 5, [6, 7, 8]];
var flat = depthArr.reduce(function (flatOutput, depthValue) {
    return flatOutput.concat(depthValue);
}, []);
console.log(flat);
...
var arrCourses = topics.reduce(function (course, topic) {
